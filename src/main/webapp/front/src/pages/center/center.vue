    ...
    <el-tabs tab-position="left" :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px 0 0 0","borderRadius":"8px","background":"rgba(255,255,255,1)"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="80px">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='yonghu'" label="用户手机" prop="yonghushouji">
            <el-input v-model="sessionForm.yonghushouji" placeholder="用户手机" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="家政账号" prop="jiazhengzhanghao">
            <el-input v-model="sessionForm.jiazhengzhanghao" placeholder="家政账号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.mima" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="家政姓名" prop="jiazhengxingming">
            <el-input v-model="sessionForm.jiazhengxingming" placeholder="家政姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="性别">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="年龄" prop="nianling">
            <el-input v-model="sessionForm.nianling" placeholder="年龄" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="联系电话" prop="lianxidianhua">
            <el-input v-model="sessionForm.lianxidianhua" placeholder="联系电话" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="图片">
            <el-upload
              class="avatar-uploader"
              :action="uploadUrl"
              :headers="headers"
              :show-file-list="false"
              :on-success="jiazhengrenyuantupianHandleAvatarSuccess">
              <img :style='{"border":"1px solid #9ce0b5","cursor":"pointer","minHeight":"100px","boxShadow":"inset 0px 0px 48px 0px #cef1db","color":"#3db769","borderRadius":"6px","textAlign":"center","width":"150px","fontSize":"32px","lineHeight":"100px","height":"auto"}' v-if="sessionForm.tupian" :src="baseUrl + sessionForm.tupian" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="工作经验" prop="gongzuojingyan">
            <el-input v-model="sessionForm.gongzuojingyan" placeholder="工作经验" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="民族" prop="minzu">
            <el-input v-model="sessionForm.minzu" placeholder="民族" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 4px","background":"none"}' v-if="userTableName=='jiazhengrenyuan'" label="最高学历" prop="zuigaoxueli">
            <el-input v-model="sessionForm.zuigaoxueli" placeholder="最高学历" ></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'jiazhengzhanghao', null);
      }
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'mima', null);
      }
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'jiazhengxingming', null);
      }
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'xingbie', null);
      }
      ...
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'gongzuojingyan', null);
      }
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'minzu', null);
      }
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.sessionForm, 'zuigaoxueli', null);
      }
      ...
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.rules, 'jiazhengzhanghao', [{ required: true, message: '请输入家政账号', trigger: 'blur' }]);
      }
      if ('jiazhengrenyuan' == this.userTableName) {
        this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
      }
    ...
    methods: {
      ...
      jiazhengrenyuantupianHandleAvatarSuccess(res, file) {
        if (res.code == 0) {
          if ('jiazhengrenyuan' == this.userTableName) {
