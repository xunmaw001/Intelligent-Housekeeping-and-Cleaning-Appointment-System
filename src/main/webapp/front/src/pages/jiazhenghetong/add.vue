...
<div :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px auto 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="服务金额" prop="fuwujine">
            <el-input v-model="ruleForm.fuwujine" 
                placeholder="服务金额" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="合同编号" prop="hetongbianhao">
            <el-input v-model="ruleForm.hetongbianhao" 
                placeholder="合同编号" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="合同文件" prop="hetongwenjian">
            <file-upload
            tip="点击上传合同文件"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}'  label="签订状态" prop="qiandingzhuangtai">
            <el-select v-model="ruleForm.qiandingzhuangtai" placeholder="请选择签订状态"  >
              <el-option
                  v-for="(item,index) in qiandingzhuangtaiOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="合同内容" prop="hetongneirong">
            <editor 
...
<script>
  export default {
    data() {
      return {
        ...
        rules: {
          ...
          qiandingzhuangtai: [
            { required: true, message: '签订状态不能为空', trigger: 'blur' },
          ],
          crossuserid: [
          ],
          crossrefid: [
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='qiandingzhuangtai'){
              this.ruleForm.qiandingzhuangtai = obj[o];
              this.ro.qiandingzhuangtai = true;
              continue;
            }
            if(o=='crossuserid'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('jiazhenghetong/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('jiazhenghetong/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('jiazhenghetong/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
